...
/* in file actuatorlowlevel.c */
extern	void actuator_write(uint32 , uint32);

/* in file actuatorputc.c */
extern	devcall	actuatorputc(struct dentry *, char);

/* in file digitallowlevel.c */
extern	uint32	sensor_read(uint32 pin);

/* in file digitalgetc.c */
extern	devcall	digitalgetc(struct dentry *);

/* in file analoginit.c */
devcall analoginit(struct dentry *);

/* in file analoggetc.c */
extern devcall analoggetc(struct dentry *);
...
/* ACTUATOR ON OFF functions*/
extern void actuatoron(did32 descrp);

extern void actuatoroff(did32 descrp);

/* ANALOG READ */
extern uint32 analog_read(struct dentry *);

/*	UDP	*/
extern process udp_edge_send(char*,uint32);
extern process udp_edge_recv(void);
...
/*	LED UDP	*/
process led_recv(uint32,uint32,uint16);

/*	LED MULTIPLE UDP	*/
process led_multiple_recv(void);

/*	REGULAR UPDATES TEMP SENSOR	*/
process regular_updates(uint32,uint32,uint16);
